...
- Band Selection Methods (SWGMF(30), E-FDPC(2.5), None) - Adjust params as needed
...
try:
    ...
    # --- MODIFICATION: Set Dataset to Botswana ---
    base_cfg_module.DATASET_NAME = 'Botswana'
    
    # Re-run dependent config logic based on the new DATASET_NAME
    if base_cfg_module.DATASET_NAME == 'Botswana':
        base_cfg_module.DATA_PATH_RELATIVE = 'botswana/'
        base_cfg_module.DATA_FILE = 'Botswana.mat'
        base_cfg_module.GT_FILE = 'Botswana_gt.mat'
        base_cfg_module.NUM_CLASSES = 14
        base_cfg_module.EXPECTED_DATA_SHAPE = (1476, 256, 145)
        base_cfg_module.EXPECTED_GT_SHAPE = (1476, 256)
        ...
        base_cfg_module.DATA_MAT_KEY = 'Botswana'
        base_cfg_module.GT_MAT_KEY = 'Botswana_gt'
    # Add other dataset configs here if this script were to be made generic
    # else:
    #     pass # Or raise error if dataset not Botswana
    ...
    from src.model import DSSFN # Assuming model.py is in src
...
#        Sweep Parameters
# ==================================
# Adjust band selection parameters if needed for Botswana
band_selection_options = [
    {'method': 'SWGMF', 'param': 30},    # Example: Target 30 bands for SWGMF
    {'method': 'E-FDPC', 'param': 2.5}, # Example: 2.5% for E-FDPC
...
fusion_mechanism_options = [
    'AdaptiveWeight' # Only AdaptiveWeight as per previous requests
]
...
BASE_RANDOM_SEED = base_cfg_module.RANDOM_SEED # From config
...
sweep_output_dir_name = f"sweep_combinations_botswana_{sweep_timestamp}"
sweep_output_dir = os.path.join(project_root, base_cfg_module.OUTPUT_DIR, base_cfg_module.DATASET_NAME, sweep_output_dir_name)
os.makedirs(sweep_output_dir, exist_ok=True)
log_file_path = os.path.join(sweep_output_dir, "sweep_combinations_botswana_log.txt")
...
logging.info(f"========== Starting Botswana Combination Sweep Run (K-Fold CV) ==========")
...
logging.info(f"Output Directory: {sweep_output_dir}")
...
# ==================================
# Pipeline Functions (Copied from sweep_combinations_pu.py, minor logging changes if any)
# ==================================
...
def train_evaluate_model(model, criterion, optimizer, scheduler, loaders, device, cfg_dict):
    ...
    if train_loader and len(train_loader.dataset) > 0:
        ...
        trained_model, history = engine.train_model(
            ...
            save_best_model=cfg_dict.get('SAVE_BEST_MODEL', True),
            early_stopping_enabled=cfg_dict.get('EARLY_STOPPING_ENABLED', False),
            early_stopping_patience=cfg_dict.get('EARLY_STOPPING_PATIENCE', 10),
            early_stopping_metric=cfg_dict.get('EARLY_STOPPING_METRIC', 'val_loss'),
            early_stopping_min_delta=cfg_dict.get('EARLY_STOPPING_MIN_DELTA', 0.0001)
        )
    ...
    try:
        ...
        for fold in range(K_FOLDS):
            ...
            for combo_idx, (band_option, intermediate_attention, fusion_mechanism) in enumerate(parameter_combinations):
                ...
                input_bands_run = B_original # Initialize
                ...
                try:
                    ...
                    train_loader, val_loader, test_loader = loaders # val_loader is passed to train_evaluate_model
                    ...
                    logging.error(f"Error in run (Fold {fold+1}, Combo {combo_idx+1}): {e_run}", exc_info=True)
        ...
        logging.info("\n\n========== All Folds and Combinations Finished for Botswana ==========")
        if all_run_results:
            df_detailed = pd.DataFrame(all_run_results)
            df_detailed['Band Param'] = df_detailed['Band Param'].fillna('N/A')
            detailed_csv_path = os.path.join(sweep_output_dir, f"sweep_combinations_botswana_detailed_results_{sweep_timestamp}.csv")
            ...
            if not df_valid_eval.empty:
                ...
                logging.info("Aggregated Results (Botswana):\n" + df_aggregated.to_string())
                
                agg_csv_path = os.path.join(sweep_output_dir, f"sweep_combinations_botswana_aggregated_results_{sweep_timestamp}.csv")
                df_aggregated.to_csv(agg_csv_path, index=False)
                
                agg_txt_path = os.path.join(sweep_output_dir, f"sweep_combinations_botswana_aggregated_results_{sweep_timestamp}.txt")
                with open(agg_txt_path, 'w') as f:
                    f.write(f"Aggregated Results (Botswana Dataset) - Timestamp: {sweep_timestamp}\n")
                    f.write(f"K-Folds: {K_FOLDS}, Train Ratio: {FOLD_TRAIN_RATIO}, Val Ratio: {FOLD_VAL_RATIO}\n\n")
                    f.write(format_results_table(df_aggregated.copy()))
                logging.info(f"Aggregated results saved to CSV and TXT.")
            else: 
                logging.warning("No runs completed successfully for aggregation.")
        else: 
            logging.warning("No results collected during the sweep.")

    except Exception as e:
        logging.error(f"Critical error in Botswana sweep: {e}", exc_info=True)
        if all_run_results: # Attempt to save any partial results
            try:
                pd.DataFrame(all_run_results).to_csv(os.path.join(sweep_output_dir, f"sweep_combinations_botswana_DETAILED_ERROR_{sweep_timestamp}.csv"), index=False)
                logging.info(f"Saved partial detailed results after error.")
            except Exception as save_e:
                logging.error(f"Could not save partial results after error: {save_e}")
        sys.exit(1)
    finally:
        logging.info(f"\nTotal Botswana sweep execution time: {(time.time() - overall_start_time)/3600:.2f} hours")
        logging.info("========== Botswana Combination Sweep Run Finished ==========")
